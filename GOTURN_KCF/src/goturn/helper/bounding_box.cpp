...
void BoundingBox::getRect(cv::Rect targetBox){
  x1_ = targetBox.x;
  y1_ = targetBox.y;
  x2_ = targetBox.x+targetBox.width;
  y2_ = targetBox.y+targetBox.height;
}


void BoundingBox::putRect(cv::Rect &targetBox) {

  targetBox.x = (int)x1_;
  targetBox.y = (int)y1_;
  targetBox.width = (int)x2_ - targetBox.x;
  targetBox.height = (int)y2_ - targetBox.y;
}
